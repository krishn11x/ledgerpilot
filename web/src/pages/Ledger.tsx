  ...
  if (ledger.data.items.length === 0) {
    return (
      <div className="mx-auto max-w-3xl space-y-4">
        <header>
          <p className="text-xs uppercase tracking-[0.2em] text-accent">Accounting</p>
        ...
        <div className="border border-border-subtle bg-surface p-6 text-sm text-ink-muted">
          No journal entries yet. Entries appear when an exception is approved or otherwise requires an accounting action.
        </div>
      </div>
    );
  }
